...
NPM := $(shell yarn --version >$(NULL) 2>&1 && echo yarn|| echo npm)
...
~install: $(DONE)/npm-install $(DONE)/composer-install
+install: _install $(DONE)/npm-install $(DONE)/composer-install
_install:
	-@rm -rf $(DONE)/install $(NOFAIL)
$(DONE)/npm-install: package.json
	@$(NPM) install
	@$(call done,npm-install)
$(DONE)/composer-install: composer.json
	@composer install
	@$(call done,composer-install)
...
dist:
	...
	# -@rm -r node_modules/.tmp/dist node_modules/.tmp/dist $(NOFAIL)
	# @$(BABEL) --env-name umd src -d dist --extensions '.ts,.tsx' --source-maps
